...
>
> Upload da era do frontend separado: sobe um diretório `Frontend/` (`:81`, `:182`) que **não
> existe** neste repositório e trata `bhub.db` como banco criado na VPS (`:154`). O caminho de
...
```bash
# Upload do frontend
scp -r Frontend usuario@seu-ip-vps:/var/www/bhub/frontend
```
...
rsync -avz --progress \
  ...
  --exclude '.next' \
  --exclude '.env.local' \
  Frontend/ usuario@seu-ip-vps:/var/www/bhub/frontend/
```

...
async def get_detailed_stats(
    ...
    category_stats = await db.execute(
        select(Category.name, func.count(Article.id).label("count"))
        .outerjoin(Article, Article.category_id == Category.id)
        .group_by(Category.id)
        .order_by(func.count(Article.id).desc())
    )

    categories = [{"name": row[0], "count": row[1]} for row in category_stats.fetchall()]

    # Top feeds por artigos
    feed_stats = await db.execute(
        select(Feed.name, func.count(Article.id).label("count"))
    ...
    top_feeds = [{"name": row[0], "count": row[1]} for row in feed_stats.fetchall()]

    # Top autores
    author_stats = await db.execute(
        select(Author.name, Author.article_count).order_by(Author.article_count.desc()).limit(10)
    )

    top_authors = [{"name": row[0], "count": row[1]} for row in author_stats.fetchall()]

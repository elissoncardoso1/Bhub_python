(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function showErrorToast(message, title = "Erro", duration = 5000) {
      ...
      // Remover automaticamente após duração especificada (sincronizado com progress bar)
      setTimeout(() => {
        toast.remove();
      }, duration);
    }
    ...
    function showSuccessToast(message, title = "Sucesso", duration = 5000) {
      const container = document.getElementById("toast-container");
      if (!container) return;
...
// Funções globais para ações do card e página de detalhe
(function() {
  'use strict';

  // Copiar DOI
  window.copyDOI = function(doi) {
    if (!doi) return;

    const doiUrl = `https://doi.org/${doi}`;
    navigator.clipboard.writeText(doiUrl).then(() => {
      if (window.showSuccessToast) {
        window.showSuccessToast('DOI copiado para a área de transferência!');
      } else {
      ...
      if (window.showErrorToast) {
        window.showErrorToast('Erro ao copiar DOI');
      }
    });
  };

  // Compartilhar artigo
  window.shareArticle = function(articleId, articleTitle) {
    const url = window.location.origin + '/articles/' + articleId;
  ...
  // Salvar artigo (placeholder - implementar conforme necessário)
  window.toggleSaveArticle = function(articleId) {
    // TODO: Implementar lógica de salvar artigo
    console.log('Salvar artigo:', articleId);
    if (window.showInfoToast) {
      window.showInfoToast('Funcionalidade de salvar em desenvolvimento');
    }
  };
})();
